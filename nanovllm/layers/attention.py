        ...
        if context.is_prefill:
            ...
            for i in range(batch_size):
                ...
                if context.block_tables is not None:
                    ...
                    k_context_len = (
                        context.cu_seqlens_k[i + 1] - context.cu_seqlens_k[i]
                    ).item()
                    ...
                    if seq_len_k == 0:
                        ...
                        physical_block_nums = context.block_tables[
                            i, block_idx_for_token
                        ]
                        offset_in_block = token_indices % self.block_size
                        slot_indices = (
                            physical_block_nums * self.block_size + offset_in_block
                        )

                        # Gather and reshape from (S_k, N_kv*H) to (S_k, N_kv, H)
                        k_seq_flat = k_cache[
                            physical_block_nums, offset_in_block
                        ]  # LLM generated k_cache[slot_indices] which is incorrect?
                        v_seq_flat = v_cache[physical_block_nums, offset_in_block]
                        k_seq = k_seq_flat.view(
                            seq_len_k, self.num_kv_heads, self.head_dim
                        )
                        v_seq = v_seq_flat.view(
                            seq_len_k, self.num_kv_heads, self.head_dim
                        )
                    ...
                    k_start, k_end = (
                        context.cu_seqlens_k[i],
                        context.cu_seqlens_k[i + 1],
                    )
                ...
                k_seq_rep = (
                    k_seq.unsqueeze(2)
                    .expand(-1, -1, self.num_q_per_kv, -1)
                    .reshape(seq_len_k, self.num_heads, self.head_dim)
                )
                v_seq_rep = (
                    v_seq.unsqueeze(2)
                    .expand(-1, -1, self.num_q_per_kv, -1)
            ...
            token_indices = (
                torch.arange(max_seq_len_k, device=q.device)
                .unsqueeze(0)
                .expand(batch_size, -1)
            )
            ...
            physical_block_nums = torch.gather(
                context.block_tables, 1, block_idx_for_token
            )
            ...
            k_valid_flat = k_cache[
                block_indices, offsets_in_block
            ]  # (num_valid_tokens, num_kv_heads, head_dim)
            v_valid_flat = v_cache[
                block_indices, offsets_in_block
            ]  # (num_valid_tokens, num_kv_heads, head_dim)
            ...
            k_past = k_past2.view(
                batch_size, max_seq_len_k, self.num_kv_heads, self.head_dim
            )
            v_past = v_past2.view(
                batch_size, max_seq_len_k, self.num_kv_heads, self.head_dim
            )
            ...
            k_past_rep = (
                k_past.unsqueeze(3)
                .expand(-1, -1, -1, self.num_q_per_kv, -1)
                .reshape(batch_size, max_seq_len_k, self.num_heads, self.head_dim)
            )
            v_past_rep = (
                v_past.unsqueeze(3)
                .expand(-1, -1, -1, self.num_q_per_kv, -1)
            ...
            o_b = F.scaled_dot_product_attention(
                q_b, k_b, v_b, attn_mask=attn_mask, scale=self.scale, is_causal=False
            )
